...
public class WebCheckControlDepictor<C extends CheckControl> extends AbstractWebComponentDepictor<C>
{
	...
	If no ID has been assigned to the given group, one will be generated and associated with the group.
...
//TODO important: change to a method of mutual exclusion determination that works for nested value models, such as are used with value prototypes
	...
	For XHTML radio buttons, a unique identifier for its mutual exclusion policy model group is returned; the component ID is returned if this is not a mutual exclusion component.
	@param checkControl The component for which a name should be retrieved.
	@return An identifier appropriate for the name attribute of the component's XHTML element, or <code>null</code> if the component's element should not have a name.
	@see MutualExclusionPolicyModelGroup
	*/
