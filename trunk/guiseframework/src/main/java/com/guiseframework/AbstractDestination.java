...
public abstract class AbstractDestination extends BoundPropertyObject implements Destination
{
	...
	/**
	 * Determines the path to use for the requested path. If there is a preferred path, it is returned; otherwise, the path is returned unmodified. If the given
	 * path does not exist but the path would exist if made into a a collection path by adding a an ending slash, that new path is returned.
	...
	/**
	 * Determines if the given location does indeed exist for this destination. This version assumes that all paths exist at this destination and returns
	 * <code>true</code>.
	 * @param session The current Guise session.
	...
	/**
	 * Determines if this destination is equivalent to the given object. This implementation considers destinations of identical types with identical paths and
	 * path patterns to be equivalent.
