...
///           description base directory, but with a `-.tupr` extension added to whatever extension the filename already has. Determination of the
///           sidecar is configurable using [#setDescriptionFileSidecarPrefix(String)] and [#setDescriptionFileSidecarExtension(String)].
...
public class SiteDirResourceSet extends DirResourceSet {
	...
	/// Returns the absolute complete path to the base of the description file tree as a [File]. Includes the configured description base and the internal
	/// path.
	/// @implNote This method is analogous to [#getFileBase()], but for the description sidecar files.
	/// @return The absolute complete path to the base of the description file tree as a [File]; includes the configured description base and the internal
	///         path.
	...
	/// @return The absolute path of the root directory of the description metadata tree, or `null` if not defined, indicating that resource set base
	///         should be used.
	/// @see #getBase()
	...
	/// @apiNote This constructor is necessary if used with the [Digester].
	public SiteDirResourceSet() {
		super();
	}

	/// A Guise-aware site-based set of resources based upon a site directory.
	/// @param root The [WebResourceRoot] this new resource set will be added to.
	...
	///          application, the directory would be mounted at `"WEB-INF/lib/"`.
	...
	/// {@inheritDoc}
	/// @implSpec If creating a [FileResource], this detects Guise metadata and uses it to initialize the resource with custom Internet media type
	///           information.
	@Override
