...
/**
 * Abstract implementation of a composite component that keeps track of its child components at specific indices in an array. The array is indexed using the
 * provided enum. Child components should not directly call {@link #addComponent(Component)} and {@link #removeComponent(Component)}. Each index in the array
 * can be <code>null</code>. Iterating over child components is thread safe.
 * <p>
 * This class should be constructed using <code>Enum.values()</code> to indicate the possible enum values.
 * </p>
...
public abstract class AbstractEnumCompositeComponent<E extends Enum<E>> extends AbstractArrayCompositeComponent {
	...
	/**
	 * Sets the component for the given enum value. If the new component is the same as the old, no action is taken. This implementation calls
	 * {@link #addComponent(Component)} and {@link #removeComponent(Component)} as necessary.
