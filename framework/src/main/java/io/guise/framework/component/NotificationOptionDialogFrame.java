...
public class NotificationOptionDialogFrame extends AbstractOptionDialogFrame<Notification.Option> {
	...
	/**
	 * Our input strategy for mapping the "continue" command to the first non-fatal option. As the options are created in the parent constructor, we'll have to
	 * lazily create this instance.
	 */
	...
	/**
	 * Text constructor with a default {@link Text#PLAIN_MEDIA_TYPE} content type. Duplicate options are ignored.
	...
	public NotificationOptionDialogFrame(final String text, final Notification.Option... options) {
		this(text, Text.PLAIN_MEDIA_TYPE, options); //construct the class with a plain text content type
	}
	...
	/**
	 * Opens the frame as modal with a {@link Runnable} to be performed after modality ends successfully. When the frame closes, if a non-fatal option was chosen
	 * the given runnable is executed. This is a convenience method that delegates to {@link Frame#open(com.globalmentor.beans.GenericPropertyChangeListener)}. If
	 * the selected option to any notification is fatal, the specified logic, if any, will not be performed. The absence of an option selection is considered
	 * fatal only if a fatal option was presented for a given notification.
