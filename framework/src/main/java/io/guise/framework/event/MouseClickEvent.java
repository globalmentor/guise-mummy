...
import static io.guise.framework.geometry.Point.*;
import static io.guise.framework.geometry.Rectangle.*;
...
public class MouseClickEvent extends AbstractMouseEvent {
	 ...
	 */
	public MouseClickEvent(final Object source, final Rectangle targetBounds, final Rectangle viewportBounds, final Point mousePosition,
			final MouseButton button, final int count, final Key... keys) {
	 ...
	 * @throws IllegalArgumentException if the given count is zero or less.
	 */
	public MouseClickEvent(final Object source, final Object target, final Rectangle targetBounds, final Rectangle viewportBounds, final Point mousePosition,
			final MouseButton button, final int count, final Key... keys) {
	...
	public MouseClickEvent(final Object source, final MouseClickInput mouseClickInput) {
		this(source, EMPTY_RECTANGLE, EMPTY_RECTANGLE, ORIGIN_POINT, mouseClickInput.getButton(), mouseClickInput.getCount(), mouseClickInput.getKeys().toArray(
				new Key[mouseClickInput.getKeys().size()])); //construct the class with the specified source		
	}
	...
	public MouseClickEvent(final Object source, final MouseClickEvent mouseClickEvent) {
		this(source, mouseClickEvent.getTarget(), mouseClickEvent.getTargetBounds(), mouseClickEvent.getViewportBounds(), mouseClickEvent.getMousePosition(),
