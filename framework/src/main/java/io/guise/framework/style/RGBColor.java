...
public class RGBColor extends AbstractModeledColor<RGBColor.Component> {
	 ...
	 * </ul>
	 * This method also recognizes the <code>transparent</code> color name as equivalent to rgba(0, 0, 0, 0), or black with zero alpha. In most instances, the
	 * {@link #valueOf(CharSequence)} static method is preferred for string-based construction, as it allows singleton instances of named RGB colors to be used.
	 * @param charSequence The character sequence representation of a color, either a lowercase name of a standard HTML color, or a three or six-digit hex code
	 *          beginning with '#'.
	...
	/**
	 * A regular expression pattern matching <code>rgb(<var>red</var>,<var>green</var>,<var>blue</var>)</code>, with the first three groups representing the three
	 * RGB character sequences.
	 */
	private static final Pattern RGB_ABSOLUTE_FUNCTION_PATTERN = Pattern.compile("rgb\\((\\d{0,3}),\\s*(\\d{0,3}),\\s*(\\d{0,3})\\)");
	/**
	 * A regular expression pattern matching <code>rgb(<var>red</var>%,<var>green</var>%,<var>blue</var>%)</code>, with the first three groups representing the
	 * three RGB character sequences.
	 */
	private static final Pattern RGB_PERCENT_FUNCTION_PATTERN = Pattern.compile("rgb\\(([\\d\\.]+)%,\\s*([\\d\\.]+)%,\\s*([\\d\\.]+)%\\)");
	/**
	 * A regular expression pattern matching <code>rgba(<var>red</var>,<var>green</var>,<var>blue</var>,<var>alpha</var>)</code>, with the first four groups
	 * representing the three RGB character sequences.
	 */
	...
	public static RGBColor valueOf(final CharSequence charSequence) {
		...
		if(rrggbbMatcher.matches()) { //if the character sequence matches
			return new RGBColor(Integer.parseInt(rrggbbMatcher.group(1), 16), Integer.parseInt(rrggbbMatcher.group(2), 16), Integer.parseInt(rrggbbMatcher.group(3),
					16)); //extract the RGB values and return a new color
		}
		...
		if(rgbAbsoluteFunctionMatcher.matches()) { //if the character sequence matches
			return new RGBColor(Integer.parseInt(rgbAbsoluteFunctionMatcher.group(1), 10), Integer.parseInt(rgbAbsoluteFunctionMatcher.group(2), 10),
					Integer.parseInt(rgbAbsoluteFunctionMatcher.group(3), 10)); //extract the RGB values and return a new color
		}
		...
		if(rgbPercentFunctionMatcher.matches()) { //if the character sequence matches
			return new RGBColor(Double.parseDouble(rgbPercentFunctionMatcher.group(1)) / 100, Double.parseDouble(rgbPercentFunctionMatcher.group(2)) / 100,
					Double.parseDouble(rgbPercentFunctionMatcher.group(3)) / 100); //extract the RGB values and return a new color
		}
		...
		if(rgbaAbsoluteFunctionMatcher.matches()) { //if the character sequence matches
			return new RGBColor(Integer.parseInt(rgbaAbsoluteFunctionMatcher.group(1), 10), Integer.parseInt(rgbaAbsoluteFunctionMatcher.group(2), 10),
					Integer.parseInt(rgbaAbsoluteFunctionMatcher.group(3), 10), Integer.parseInt(rgbaAbsoluteFunctionMatcher.group(4), 10)); //extract the RGBA values and return a new color
		}
		...
		if(rgbaPercentFunctionMatcher.matches()) { //if the character sequence matches
			return new RGBColor(Double.parseDouble(rgbaPercentFunctionMatcher.group(1)) / 100, Double.parseDouble(rgbaPercentFunctionMatcher.group(2)) / 100,
					Double.parseDouble(rgbaPercentFunctionMatcher.group(3)) / 100, Double.parseDouble(rgbaPercentFunctionMatcher.group(4)) / 100); //extract the RGBA values and return a new color
		}
