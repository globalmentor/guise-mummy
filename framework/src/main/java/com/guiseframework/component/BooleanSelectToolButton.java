...
/**
 * Button that stores a Boolean value in its model representing the selected state. A validator requiring a non-<code>null</code> value is automatically
 * installed. Tool buttons are typically presented on toolbars and rendered differently than a normal button; they usually are more subtle and may only present
 * button decorations upon certain gestures such as mouse overs.
...
public class BooleanSelectToolButton extends AbstractBooleanSelectActionControl implements SelectButtonControl, ToolButtonControl {
	 ...
	 */
	public BooleanSelectToolButton(final InfoModel infoModel, final ActionModel actionModel, final ValueModel<Boolean> valueModel, final Enableable enableable) {
