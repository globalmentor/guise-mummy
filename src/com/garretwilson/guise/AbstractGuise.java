...
public abstract class AbstractGuise<GC extends GuiseContext> implements Guise<GC>
{
	...
	*/
	...
	{
		return (Class<? extends Controller<GC, C>>)renderStrategyMap.put(componentClass, renderStrategyClass);	//register the render strategy and return the old registration, if any
//TODO fix		return (Class<? extends RenderStrategy<? super GC, ? extends C>>)renderStrategyMap.put(componentClass, renderStrategyClass);	//register the render strategy and return the old registration, if any
	...
	*/
	...
	{
		return (Class<? extends Controller<GC, C>>)renderStrategyMap.get(componentClass);	//return any registration; we know that the render strategy is for the correct type, because that's the only type we'll allow to be registered
