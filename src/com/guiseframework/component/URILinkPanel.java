...
/**A panel displaying a URI as a series of links for each path component,
with each link target representing that path component relative to the application.
Individual path components are unencoded when displayed in their respective links.
@author Garret Wilson
*/
public class URILinkPanel extends AbstractPanel<URILinkPanel>
{

	/**The URI bound property.*/
	public final static String URI_PROPERTY=getPropertyName(URILinkPanel.class, "uri");
		...
		*/
		public void setURI(final URI newURI)
		{
			if(!ObjectUtilities.equals(uri, newURI))	//if the value is really changing
			{
				...
				final StringBuilder hrefStringBuilder=new StringBuilder();	//create a string builder to collect the components of the URI
				...
				if(!isPathURI(uri))	//if the URI is not just a path
				{
					hrefStringBuilder.append(changePath(uri, null));	//initialize the href with a path-less URI TODO eventually encode
					add(new Link(decode(hrefStringBuilder.toString()), isAbsolutePath ? hrefStringBuilder.toString()+PATH_SEPARATOR : hrefStringBuilder.toString()));	//add a link for the domain, adding a root path to the href if the URI has an absolute path
				}
				...
				while(pathTokenizer.hasMoreTokens())	//while there are more tokens
				{
					...
					{
						add(new Link(decode(token), pathTokenizer.hasMoreTokens() ? hrefStringBuilder.toString()+PATH_SEPARATOR : hrefStringBuilder.toString()));	//add a link for this component, appending a path separator to the href if there is a path separator following this path component (that's all that can follow this component, as the path separator is the delimiter)
					}
				}
	...
	*/
	public URILinkPanel(final URI uri)
	{
	...
	*/
	public URILinkPanel(final Layout<?> layout)
	{
	...
	@exception NullPointerException if the given layout is <code>null</code>.
	*/
	public URILinkPanel(final Layout<?> layout, final URI uri)
	{
