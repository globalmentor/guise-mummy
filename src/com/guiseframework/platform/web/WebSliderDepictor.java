...
//TODO fix			writeClassAttribute(context, closeStyleIDs);	//write the title style IDs
			...
			depictContext.writeAttribute(null, ELEMENT_IMG_ATTRIBUTE_SRC, session.resolveURI(trackImageURI).toString());	//src="trackImageURI"
...
//TODO fix			writeClassAttribute(context, closeStyleIDs);	//write the title style IDs
				//use the correct flow axis suffix when resolving URIs
			depictContext.writeAttribute(null, ELEMENT_IMG_ATTRIBUTE_SRC, session.resolveURI(thumbImageURI).toString());	//src="thumbImageURI"
			depictContext.writeElementEnd(XHTML_NAMESPACE_URI, ELEMENT_IMG);	//</xhtml:img>
		}
...
//TODO write the class and style					
					...
					final BigDecimal bigPercent=flowDirection==Flow.Direction.INCREASING ? bigPosition.multiply(bigDecimal100) : bigDecimal1.subtract(bigPosition).multiply(bigDecimal100);	//get the position along the slider, compensating for flow direction
