...
public abstract class AbstractWebLayoutComponentDepictor<C extends LayoutComponent> extends AbstractWebComponentDepictor<C>
{
	...
	*/
	...
	@exception IOException if there is an error updating child views.
	@exception IllegalArgumentException if the component's layout is not supported.
	...
	*/
	protected void depictChildren() throws IOException
	{
		...
		boolean useMozInlineBoxFixup=false;	//decide whether we need to add an extra surrounding container to help Mozilla inline boxes
		final WebUserAgentProduct userAgent=getPlatform().getClientProduct();	//get the user agent
		if(userAgent.getBrand()==WebUserAgentProduct.Brand.FIREFOX && userAgent.getVersionNumber()<3)	//if this is Firefox<3, we may need to add an extra container <div> to prevent text from escaping for -moz-inline-box children
		{
			final CompositeComponent parentComponent=component.getParent();	//get the parent component, if any
			if(parentComponent instanceof LayoutComponent)	//if we're inside a layout component
			{
				final Layout<?> layout=((LayoutComponent)parentComponent).getLayout();	//get the parent layout
				if(layout instanceof FlowLayout)	//if the parent has a flow flow layout
				{
					final FlowLayout flowLayout=((FlowLayout)layout);	//get the flow layout
					final Flow flow=flowLayout.getFlow();	//get the flow of the flow layout
					if(flowLayout.isWrapped() && parentComponent.getComponentOrientation().getAxis(flow)==Axis.X)	//if the parentcomponent is in a wrapped flow layout flowing on the X axis, perform a fixup for -moz-inline-box
					{
						final Extent width=component.getExtent(flow);	//get the extent specified for the flow for this component
						if(width!=null)	//if we have a specified width for this component, place another block <div> with the same width
						{
						...
						case Y:	//if we should span the left and right components vertically
							{
								...
								for(; centerColumnRowIndex<columnComponents.length && columnComponents[centerColumnRowIndex]==null; ++centerColumnRowIndex);	//skip the null center column components
...
*/
		...
		if(useMozInlineBoxFixup)	//if we're using the special fixup for Mozilla inline boxes, write the <div> end tag
		{
			depictContext.writeElementEnd(XHTML_NAMESPACE_URI, ELEMENT_DIV);	//</xhtml:div>
		}
	}
