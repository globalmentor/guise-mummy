...
/**
 * Strategy for rendering an action control as an XHTML <code>&lt;button&gt;</code> element. This depictor allows rollovers and only renders the border upon
 * rollover.
	...
	protected Map<String, Object> getBodyStyles() {
		...
		if(component.isRollover()) { //if the component is in a rollover state
			...
			for(final Side side : CSS_SIDES) { //for each side
				...
				styles.put(CSS_PROPERTY_BORDER_X_COLOR_TEMPLATE.apply(getSerializationName(side)), parentBackgroundColor != null ? parentBackgroundColor
						: RGBColor.TRANSPARENT); //set the border color to the same color as the parent (if there is no parent, use a transparent border, but there should always be a parent)
