...
/**
 * Abstract implementation of a composite component that keeps track of its child components at specific indices in an array. Child components should not
 * directly call {@link #addComponent(Component)} and {@link #removeComponent(Component)}. Each index in the array can be <code>null</code>. Iterating over
 * child components is thread safe.
...
public abstract class AbstractArrayCompositeComponent extends AbstractMultipleCompositeComponent {
	 ...
	 */
	protected Component getComponent(final int index) {
		return componentArray[index];
	}

	/**
	 * Sets the component at the given index. If the new component is the same as the old, no action is taken. This implementation calls
