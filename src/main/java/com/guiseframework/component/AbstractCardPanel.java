...
/**
 * An abstract panel with a card layout. The component valid status is updated before a change in the {@link #VALUE_PROPERTY} or the {@link #VALIDATOR_PROPERTY}
 * is fired. If bookmarks are enabled, this component supports bookmarks using this component's name and the name of any selectected cards.
...
public abstract class AbstractCardPanel extends AbstractListSelectContainerControl implements Panel, CardControl, NavigationListener {
	...
	/**
	 * Determines the component for navigation based upon the given bookmark. A bookmark parameter with a value of the empty string is considered to indicate the
	 * <code>null</code> value. Child components may override this method to select a different bookmark based upon whether components are enabled, for example.
	...
	/**
	 * Determines the component for navigation based upon the given bookmark parameter value. A bookmark parameter with a value of the empty string is considered
	 * to indicate the <code>null</code> value. Child components may override this method to select a different bookmark based upon whether components are
	 * enabled, for example.
	 * @param bookmarkName The bookmark parameter value for which a component should be returned.
	 * @return The child component indicated by the given bookmark parameter value, or <code>null</code> if the given bookmark name represents the
	 *         <code>null</code> component value.
