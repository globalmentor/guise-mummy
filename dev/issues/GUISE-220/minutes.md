...
- 2026-06-05 **Decision**: Opener tightened from `[\r\n]+` to exactly one logical line ending (`(?:\r\n|[\r\n])`), making the YAML group start position deterministic (always line 2). This allowed the `\n` × offset prepend to be simplified to a literal `"\n"`. Rejected keeping `+` as unnecessary leniency that complicated the offset calculation.
- 2026-06-05 **Milestone**: Bug fix complete — `MARKDOWN_WITH_YAML_PATTERN` corrected with non-greedy matching, start-of-line closer anchor, and trailing-whitespace-only closer; `YamlEngineException` wrapped as `IOException` with accurate SnakeYAML line numbers.
