...
///
/// A subclass may call [#mummify(LifeCyclePhase)] to initiate testing, or testing may be done manually using the fixture project which has been set up for
/// each test.
...
public abstract class BaseEndToEndIT {
	...
	/// @implSpec This method calls [#configure(Map)] to set test-specific configuration settings.
	/// @implSpec This method calls [#populateSiteSourceDirectory(Path)] to copy files into the test site source directory in preparation for testing.
	...
	/// @apiNote These settings are later available via [#getFixtureProjectSettings()].
