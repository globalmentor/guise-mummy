...
The `Artifact` interface defines a precise vocabulary documented in its Javadoc. The categories below are organized from structural composition down to content-level concerns.
...
Not all artifacts have content in the byte-stream sense. A directory artifact is purely structural — it contains child artifacts but has no readable bytes of its own. The `CorporealSourceArtifact` interface distinguishes artifacts that _do_ have content.

- **Corporeal artifact** (`CorporealSourceArtifact`) — An artifact with readable content, accessed via `openSource(MummyContext)` and `getSourceSize(MummyContext)`. All file-based artifacts are corporeal. Importantly, "corporeal" does not mean "backed by a file on disk." An artifact can be corporeal with content that is synthesized in memory (see _phantom artifact_ below). The defining characteristic is that the artifact _has a content stream_, not where that stream comes from.

- **Phantom artifact** — A corporeal artifact whose content is generated rather than read from a source file. The canonical example is `SimpleGeneratedXhtmlArtifact`: when a directory has no explicit content file (such as `index.xhtml`), `DirectoryMummifier` creates a phantom artifact that synthesizes a minimal XHTML skeleton in memory. The phantom artifact has a source path (e.g. `foo/index.xhtml`) for identity and reference resolution, but the file at that path does not exist on disk — `openSource()` returns the generated bytes, not a file stream. The mummifier assigned to a phantom artifact (typically `XhtmlPageMummifier`) processes it like any other corporeal artifact, unaware that the content was generated rather than read from a file.

#### Visibility and Processing Designations

These designations control how an artifact participates in navigation and content processing. They are determined during planning from source filename conventions and stored on the artifact.
...
- **Asset** — A veiled artifact that additionally will not produce generated pages. Designated by source filename convention (default: dollar-sign prefix such as `$template.html`). The asset designation only suppresses page generation — it does **not** prevent other content transformations. For example, if a registered image mummifier would normally scale a large image, it will still do so for an asset image. The only mummifier override that occurs is: when a file in an asset tree would be handled by a `PageMummifier`, it is instead handled by the default file mummifier (currently `GenericFileMummifier`). All other registered mummifiers are used as normal.
...
| `OpaqueFileMummifier` | `AbstractFileMummifier` | any | File copy with no transformation |
| `GenericFileMummifier` | `AbstractFileMummifier` | any | File copy with extension-based media type detection |
...
- `getDefaultSourceFileMummifier()` → `GenericFileMummifier` (copies file unchanged, detects media type from extension)
